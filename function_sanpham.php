...
class sanpham extends ketnoi{
    ...
    public $tenanh;
    function hienthi(){
        $sql = "SELECT sanpham1.*, sanpham_nhom.tennhom FROM sanpham1 INNER JOIN sanpham_nhom ON sanpham1.nhom_id = sanpham_nhom.id ";
    ...
    function themsanpham(){
        $sql = "INSERT INTO `sanpham1`(`masp`, `nhom_id`, `tensp`, `mota`, `dongiacu`, `dongiamoi`,  `enable`, `ghichu`, `diemnoibat`, `xuatxu`, `ngaydang`, `kichthuoc`, `nguoidang`)
    VALUES ('".$this->masanpham."','".$this->nhomid."','".$this->tensanpham."','".$this->mota."','".$this->dongiacu."','".$this->dongiamoi."', '".$this->enable."','".$this->ghichu."','".$this->diemnoibat."', '".$this->xuatxu."', '".$this->ngaydang."','".$this->kichthuoc."','".$this->nguoidang."')";
    $result = mysqli_query($this->conn, $sql);
