...
function taodonhang($name, $address, $tel,$email,$total,$pttt,$tendangnhap){
    $conn=ketnoidb();
        $sql = "INSERT INTO bill (name, address, tel, email, total, pttt, tendangnhap)
        VALUES ('$name', '$address', '$tel', '$email', '$total', '$pttt', '$tendangnhap')";
        // use exec() because no results are returned
        $conn->exec($sql);
...
function getOrdersByUser($user){
    $conn = ketnoidb();
    $sql = "SELECT * FROM bill WHERE tendangnhap = :user";
